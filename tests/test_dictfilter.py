...
NAMES =  ["Adam",   "Bob",    "Charles",    "David",  "Edward",  "Frank",   "Geoff",   "Hugh",   "Ian",        "John"]
CITIES = ["London", "London", "Birmingham", "London", "Cardiff", "Glasgow", "Cardiff", "London", "Birmingham", "Birmingham"]
SALES =  [100,      400,      350,          290,      180,       320,       500,       380,      350,          460]

SOURCE_DATA_LIST = [
    {"name" : name, "city" : city, "sales" : sales} for name, city, sales in zip(NAMES, CITIES, SALES)
]


def test_filter_list():
    filter = pydictsql.DictFilter("SELECT {name}, {city} FROM {sales_data} WHERE {sales} > 250")
...
def test_filter_gen():
    filter = pydictsql.DictFilter("SELECT {name}, {sales} FROM {sales_data} WHERE {sales} > 250")
    records = 0
    names = set()
    ...
    assert(records == 8)
    for expected in ["Bob", "Charles", "David", "Frank", "Geoff", "Hugh", "Ian", "John"]:
